...
class Television:
    ...
    next: go to the immediatly higher available channel
    previous: go to the immediatly lower available channel
    off: turn off the TV
    """
    options_off = """on: turn on the TV again
    ...
    def use(self):
        ...
        while self.command != 'off':
            ...
            self.command = input("What command to you wish for the TV to execute? ").lower()

            if self.command == 'off':
                Energy.energize(Television)
            else:
                if self.command == 'next':
                    Channel.change_channel('next', self.channel)
                elif self.command == 'previous':
                    Channel.change_channel('previous', self.channel)
